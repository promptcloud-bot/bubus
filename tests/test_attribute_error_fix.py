"""Test that the AttributeError bug related to '_event_processed_at' is fixed"""
...
async def test_event_without_handlers():
    ...
    # Mark as processed manually (simulating what happens in _mark_complete_if_all_handlers_completed)
    event._mark_complete_if_all_handlers_completed()

    # After marking complete, it should be set
    # When no handlers but event is processed, event_started_at returns _event_processed_at
    assert event.event_started_at is not None  # Uses _event_processed_at
    assert event.event_completed_at is not None  # Now it's complete


def test_event_with_manually_set_processed_at():
    """Test events where _event_processed_at is manually set (like in test_log_history_tree.py)"""
    ...
    if hasattr(event, '_event_processed_at'):
        event._event_processed_at = datetime.now(UTC)

    # Should not raise AttributeError
    assert event.event_started_at is not None  # Should use _event_processed_at
    # Note: Since we set _event_processed_at and there are no handlers, event_completed_at will also return _event_processed_at
    assert event.event_completed_at is not None
