...
class TestComplexIntegration:
    ...
    async def test_eventbus_dispatch_helper(self):
        """Test the event.eventbus_dispatch() helper method"""
        bus = EventBus(name='TestBus')
        
        try:
            # Test 1: Dispatch event normally first
            event = UserActionEvent(action='test', user_id='123')
            dispatched_event = bus.dispatch(event)
            
            # Test 2: Use eventbus_dispatch() helper
            redispatched_event = event.eventbus_dispatch()
            
            # Should return the same event instance
            assert redispatched_event is event
            
            # Test 3: Try to dispatch an event that wasn't dispatched by a bus
            orphan_event = UserActionEvent(action='orphan', user_id='456')
            with pytest.raises(RuntimeError, match='not attached to any EventBus'):
                orphan_event.eventbus_dispatch()
            
            # Test 4: Test with handler that uses eventbus_dispatch
            results = []
            child_event_ref = None
            
            async def handler_that_redispatches(event: UserActionEvent):
                # Handler that tests eventbus_dispatch() inside a handler
                nonlocal child_event_ref
                if event.action == 'first':
                    # Create and dispatch a new event using the helper
                    child_event = UserActionEvent(action='child', user_id=event.user_id)
                    child_event._event_dispatched_by_eventbus = event._event_dispatched_by_eventbus
                    child_event_ref = child_event.eventbus_dispatch()
                    results.append(f'dispatched_child_{child_event.event_id}')
                    return 'first_handled'
                else:
                    results.append(f'handled_{event.action}')
                    return f'{event.action}_handled'
            
            bus.on('UserActionEvent', handler_that_redispatches)
            
            # Dispatch initial event
            initial_event = UserActionEvent(action='first', user_id='789')
            bus.dispatch(initial_event)
            await initial_event
            
            # Wait for child event to complete too
            if child_event_ref:
                await child_event_ref
            
            # Should have handled the initial event, dispatched the child, and handled the child
            assert len(results) >= 2  # At least the dispatch and child handling
            assert any('dispatched_child' in r for r in results)
            assert any('handled_child' in r for r in results)
            
            # Test 5: Test weak reference cleanup
            # Create a bus that will be garbage collected
            temp_bus = EventBus(name='TempBus')
            temp_event = UserActionEvent(action='temp', user_id='999')
            temp_bus.dispatch(temp_event)
            
            # Wait for the event to be processed
            await temp_event
            
            # Store weak ref before deleting bus
            weak_ref = temp_event._event_dispatched_by_eventbus
            assert weak_ref is not None
            
            # Stop the bus which should clear circular references
            await temp_bus.stop()
            
            # After stop(), the event's reference should be cleared
            assert temp_event._event_dispatched_by_eventbus is None
            
            # Try to dispatch - should fail with no EventBus reference
            with pytest.raises(RuntimeError, match='not attached to any EventBus'):
                temp_event.eventbus_dispatch()
